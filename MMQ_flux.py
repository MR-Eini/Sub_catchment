...
SSQ_PATH = r"E:\pythonProject\Europe\DELTAQ_90M_MMQ_FIX\MMQ_90m_3035_aligned.tif"     # SSQ raster (values on reaches)
...
OUT_DIR = r"E:\pythonProject\Europe\DELTAQ_90M_MMQ"
...
def ssq_nodata_to_zero(ssq_in, out_ras, nodata_value=-9999):
    """
    Convert SSQ NoData (and/or sentinel -9999 values) to 0.
    This prevents extracting -9999 at outlets and corrupting deltaQ.
    """
    delete_if_exists(out_ras)

    ssq = Raster(ssq_in)

    # 1) Turn true NoData -> 0
    ssq0 = Con(IsNull(ssq), 0, ssq)

    # 2) Turn sentinel value (-9999) -> 0 (works even if stored as actual pixel values)
    ssq_fixed = Con(ssq0 == nodata_value, 0, ssq0)

    ssq_fixed.save(out_ras)
    return out_ras
...
def main():
    ...
    ssq_aligned = align_raster_to_template(SSQ_PATH, STREAMLINK, ssq_aligned_path, resampling="NEAREST")

    # Convert NoData / -9999 to 0 BEFORE any sampling/extraction
    ssq_fixed_path = os.path.join(OUT_DIR, "SSQ_aligned_nodata0.tif")
    SSQ_USED = ssq_nodata_to_zero(ssq_aligned, ssq_fixed_path, nodata_value=-9999)
    ...
    tbl, from_f, to_f, new_f, scale = build_reclass_table(
        gdb, "deltaQ_table", delta, desired_scale=DELTAQ_SCALE_DESIRED
    )
